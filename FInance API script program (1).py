...
fonctions = ['BBANDS', 'RSI', 'MACD', 'STOCH', 'EMA' ]


# On charge tous les indicateurs techniques dans un tableau
...
# On charges les données des bandes de boullinger

# In[21]:


analyses_BB = analyse(titre, "BBANDS")
...
result_bb = pd.concat([df_data, analyses_BB], axis=1, join_axes=[analyses_BB.index])
...
# On charge les données du RSI

# In[24]:


analyses_rsi = analyse(titre, "RSI")


# On rajoute les données au dataframe resulat

# In[25]:


result_bb = pd.concat([result_bb, analyses_rsi], axis=1, join_axes=[analyses_rsi.index])
...
analyses_macd = analyse(titre, "MACD")


# On rajoute les données au dataframe resultat

# In[28]:


result_bb = pd.concat([result_bb, analyses_macd], axis=1, join_axes=[analyses_macd.index])
